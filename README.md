...
- `npm run visual:baseline:update`：以 1280×720 重新產生三張 baseline，並將 review status 重設為待人工審查。
...
已核准的 baseline 位於 `tests/visual/baselines/ai-governance/`，第一次人工審查紀錄位於 `artifacts/evidence/visual-review/phase-b-baseline-review-20260716.md`。

不要直接編輯 `decks/*/slides.md`。內容變更應先進入 `deck.mjs`，讓相同 semantic model 未來能同時供 Slidev 與 editable PPTX renderer 使用。

## Claim boundary

目前證明的是 semantic model、三個 layout、renderer、lint、Slidev build，以及單一 Windows/Chrome 環境下的 screenshot baseline 與人工視覺審查。它不證明完整 layout library、跨作業系統像素一致性、PDF/PPTX 匯出品質或 editable PPTX renderer 已完成。
