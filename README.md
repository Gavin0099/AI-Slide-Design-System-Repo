...
npm run pptx:build
npm run pptx:test
npm run mermaid:test
npm run mermaid:demo
```
...
- `npm run governance:projection:check`：在無法連線公司 GitLab 的 GitHub-hosted runner 上，驗證 committed submodule gitlink、`framework.lock.json`、protected file hash 與 contract 必填欄位；不宣稱 upstream freshness 或完整 framework drift。
...
受限 Mermaid 圖表使用 `## mermaid` layout，並把完整圖表放在 `### diagram` 下的單一 fenced `mermaid` block。支援 `flowchart TB|TD|BT|LR|RL`、flowchart 內的 `subgraph ... end`，以及 `sequenceDiagram`。執行 `npm run mermaid:demo` 可產生三頁範例 PPTX；`npm run mermaid:test` 會驗證語法限制、確定性 SVG、manifest digest、mutation failure 與雙 renderer 共用相同 SVG bytes。

Mermaid source 是唯一可編輯來源。SVG 與 manifest 由固定 Mermaid 11.16.0 在 `dist/mermaid-assets/` 生成，不得手改；HTML、init directive、click、任意 `classDef/style/linkStyle`、外部 URL 與可執行連結一律拒絕。Flowchart 只額外允許 `class <node> <role>` 選用固定的 `input`、`capability`、`agent`、`tool`、`repository`、`outcome`、`governance`、`boundary` 語意色票，不能夾帶 CSS。PPTX 第一階段將 SVG 視為單一向量圖片，節點級原生 PowerPoint 編輯留在第二階段。

之後新增或修改簡報內容時，只編輯 `decks/ai-governance/content.md`，再執行 `npm run content:pptx`。格式使用 `## <layout>` 切分投影片、`### <field>` 定義欄位，清單使用 Markdown bullets，結構化項目使用 ` :: ` 分隔；目前所有陣列欄位固定三筆，需要固定標題換行時使用受模型驗證的可選 `titleBreakAfter`，不要在 renderer 比對特定文案。完整 contract 見 `docs/CONTENT_MARKDOWN.md`，三槽決策與遷移方式見 `docs/CARDINALITY_CONTRACT.md`。

若輸入是已完成投影片文案的既有 Markdown，`#` 固定代表封面，每一個 `##` 固定代表一頁投影片；即使可見編號重複也不得合併。標題、子標題、段落、清單、引言、程式區塊與表格儲存格必須逐字保留，AI 只能決定排版與視覺呈現，不得摘要、改寫、補字或刪字。可直接執行：

```powershell
npm run source:pptx -- --source <source.md> --output <deck.pptx>
```

此命令在輸出後會解開 PPTX OOXML，逐頁驗證所有 audience-visible text 與來源一致。`npm run source:coverage -- --source <source.md> --deck <deck.mjs> --verbatim` 則可驗證已產生的 Semantic Model 同時符合分頁與逐字內容契約。

逐字模式仍會依內容形狀選擇視覺構圖：短內容使用 `statement`、三筆以上清單使用 `list`、一般段落使用 `narrative`、高密度內容使用 `dense`，表格與程式碼分別使用 `table` / `code`。這些 variant 只改變原生可編輯物件的排版、色塊與層級，不得加入、刪除或改寫任何可見文字。

Semantic Model 驗證會一次回報整份 deck 的所有獨立錯誤，順序固定為 deck、slide、field、item；`content.md`、Slidev 與 PPTX 路徑共用同一份錯誤集合。

`deck.description` 必須是最多 120 字的單行文字；`comparison.accent` 是受控 token，目前只允許 `governance`。直接呼叫 Semantic Model 或 renderer 也不能繞過這兩項限制。
