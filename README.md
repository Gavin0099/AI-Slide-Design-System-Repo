...
若輸入是既有報告／簡報 Markdown，`#` 固定代表封面，每一個 `##` 固定代表一頁投影片；即使可見編號重複也不得合併。規劃後的每頁以 `sourceSection` 與 `sourceHeading` 綁回來源，並用 `npm run source:coverage -- --source <source.md> --deck <deck.mjs>` 驗證沒有漏頁、拆頁、合併或重排。AI 只能在單一來源段落內選版型與濃縮文案，不能改寫來源的分頁權限。
